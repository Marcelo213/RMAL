 void PID_Position_Controller(){
  ...
  double kp_3 = 10;
  double ki_3 = 0.00004; // Using Integral tends to cause a problem where it bounces around the target angle
  double kd_3 = 0;
  ...
  /* ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    ...
    ALL controls are with respect to the global frame and are absolute positions
  ...
  */
  ...
  // Error for linear
  double x1 = (26)*get_rad_300( -1*ASSIST_LINEAR_ENCODER.read());      //Arc length of motor; 28 is in mm
  Current_Pose.x = x1;
  
  
     // position for linear actuator; in steps; 27mm for 1000 steps
  if (Target_Pose.x != x2) {        // Checks if the actuator is already in the correct position
  double temp_error = Target_Pose.x - x2;
   
  
    ACTUATOR_MOVE( temp_error *37);
  ...
  //////////////////////////////////////////////////////////////////////////////////////
  // Doing PID for bending
  Current_Pose.phi = get_rad_1000( -1*CATHETER_BENDING_ENCODER.read());      //Arc length of motor; 28 is in mm
 
  double PHI_ERROR = Target_Pose.phi - Current_Pose.phi;

    INTEGRAL_ERROR_PHI += PHI_ERROR * elapsedTime;                // compute integral
    DERIVATIVE_ERROR_PHI = (PHI_ERROR - LAST_ERROR_PHI)/elapsedTime;        // compute derivative

    LAST_ERROR_PHI = PHI_ERROR;                                //remember current error
    previousTime = currentTime;                        //remember current time

  double PID_POSITION_BENDING = kp_3*PHI_ERROR + ki_3*INTEGRAL_ERROR_PHI + kd_3*DERIVATIVE_ERROR_PHI;                //PID output; error in rad          

  // ------ Allows the motor to rotate both ways
  if (PID_POSITION_BENDING < 0){
  ...
  PID_POSITION_BENDING = abs(PID_POSITION_BENDING);
  PID_POSITION_BENDING = constrain(PID_POSITION_BENDING, 0, 2*PI);               // Resolution is being improved by increasing the constrain limits
  PID_POSITION_BENDING = map(PID_POSITION_BENDING, 0, 2*PI, 0 , 255);
  analogWrite(CATHETER_BENDING_PWM_PIN, PID_POSITION_BENDING);
