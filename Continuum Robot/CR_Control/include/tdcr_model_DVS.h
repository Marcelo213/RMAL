...
{  
	private:
		 // Constants
        static const int m_nSeg = 1;
        static const int m_nQ = 2;
        static const int m_nSPQ = 2;
		
        //Member variables defining the parameters of the TDCR
		std::array<double, m_nSeg> m_length; //Length of each segment
		std::array<double,m_nSeg> m_pradius_disks; //Pitch radius of each segment (distance between backbone and tendons)
        std::array<double, m_nQ> m_q;
	...
	public:
		TDCRModelDVS(std::array<double,m_nSeg> length, int disks_per_seg, std::array<double,m_nSeg> pradius_disks, Eigen::Matrix4d base_frame);
		~TDCRModelDVS();
